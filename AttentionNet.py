...
class Encoder(nn.Module):
    ...
    def forward(self, node_embedding, agent_embedding, mask=None):
        node_agent_embedding = 0
        ...
        for layer in self.agent_layers:
            agent_embedding = layer(agent_embedding, mask)
        for layer in self.node_agent_layers:
            node_agent_embedding = layer(node_embedding, agent_embedding, mask)
        return aggregated_embedding, agent_embedding, node_agent_embedding
...
class Decoder(nn.Module):
    ...
    def forward(self, aggregate_embedding, agent_embedding, connected_node_agent_embedding, mask):
        current_state_embedding = 0
        final_candidate_embedding = 0
        for layer in self.layers:
            current_state_embedding = layer(aggregate_embedding, agent_embedding, mask=None)
        for layer in self.layers:
            final_candidate_embedding = layer(current_state_embedding, connected_node_agent_embedding, mask=mask)
...
class AttentionNet(nn.Module):
    ...
    def graph_embedding(self, node_inputs, edge_inputs, pos_encoding, agent_input, mask=None):
        ...
        # agent embedding
        agent_embedding = self.agent_embedding(agent_input)
        ...
        aggregated_embedding, agent_embedding, node_agent_embedding = self.encoder(embedding_feature, agent_embedding)


        return aggregated_embedding, agent_embedding, node_agent_embedding

    def select_next_node(self, aggregated_embedding, agent_embedding, node_agent_embedding, edge_inputs, budget_inputs,
        ...
        embedding_dim = node_agent_embedding.size()[2]

        th = torch.FloatTensor([ADAPTIVE_TH]).unsqueeze(0).unsqueeze(0).repeat(batch_size, sample_size, 1).to(
            node_agent_embedding.device)

        # change from embedding feather in single ipp to nodes_agent_embedding
        node_agent_embedding = self.budget_embedding(torch.cat((node_agent_embedding, budget_inputs, th), dim=-1))
        # connected node feature from node_agent_embedding
        connected_nodes_agent_embedding = torch.gather(node_agent_embedding, 1,
        ...
        # current_node_feature is replaced by aggregated_embedding
        ...
        # end_node_feature = node_agent_embedding[:, 0, :].unsqueeze(1)
        ...
        if mask is not None:
            current_mask = torch.gather(mask, 1, current_index.repeat(1, 1, k_size)).to(node_agent_embedding.device)
            # print(current_mask)
        else:
            current_mask = torch.zeros((batch_size, 1, k_size), dtype=torch.int64).to(node_agent_embedding.device)
        one = torch.ones_like(current_mask, dtype=torch.int64).to(node_agent_embedding.device)
        ...
        current_feature_prime = self.decoder(current_node_feature, agent_embedding, connected_nodes_agent_embedding,
                                             current_mask)
    ...
    def forward(self, node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c,
                pos_encoding, agent_input, mask=None):
        with autocast():
            aggregated_embedding, agent_embedding, node_agent_embedding = self.graph_embedding(node_inputs, edge_inputs,
                                                                                               pos_encoding,
                                                                                               agent_input, mask=None)
            logp_list, value, LSTM_h, LSTM_c = self.select_next_node(aggregated_embedding, agent_embedding,
                                                                     node_agent_embedding, edge_inputs, budget_inputs,
                                                                     current_index, LSTM_h, LSTM_c, mask)
        return logp_list, value, LSTM_h, LSTM_c
