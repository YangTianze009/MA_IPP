...
class WorkerTest:
    ...
    def run_trajectory_sampling_episode(self, currEpisode, testID):
        ...
        node_coords, graph, node_info, node_std, budget = self.env.reset(agent_ID=testID)   #### change here !!
        np.random.seed(self.seed)
        #torch.manual_seed(self.seed)
        n_nodes = node_coords.shape[0]
        node_info_inputs = node_info.reshape((n_nodes, 1))
    ...
    def make_gif(self, path, n, testID):
        with imageio.get_writer('{}/ep{}_test{}_cov_trace_{:.4g}.gif'.format(path, n, testID, self.env.cov_trace), mode='I', duration=0.2) as writer:
