...
class RAOr:
    def __init__(self, sample_size=50, k_size=20, seed=None, show_img=False):
        ...
        self.env = Env(sample_size=sample_size, k_size=k_size, budget_range=(2.99999,3), start=(0,0), destination=(1,1), seed=seed)
        ...
        print(type(self.env.graph))
        print(self.env.graph.values())
    ...
    def run_RAOr_adaptive(self, agent_ID):
        ...
        for route_i  in final_route:
            print(route_i)
            self.global_agent_pos[f"{agent_ID}"].append(self.env.node_coords[route_i])
