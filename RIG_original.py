...
class RRT:
    def __init__(self, num_nodes = 50, XDIM = 1.0, YDIM = 1.0, radius = 0.5, sample_size = 0.05, gp_func = None, gaussian_distrib = None):
    ...
    def chooseParent(self, nn, newnode):
        for p in self.nodes:
            if self.distance([p.x,p.y], [newnode.x,newnode.y]) < self.radius and p.cost + self.distance([p.x,p.y], [newnode.x,newnode.y]) < nn.cost + self.distance([nn.x,nn.y], [newnode.x,newnode.y]): 
                nn = p
    ...
    def reWire(self, newnode):
        for i in range(len(self.nodes)):
            p = self.nodes[i]
            if p != newnode.parent and self.distance([p.x,p.y], [newnode.x,newnode.y]) < self.radius and newnode.cost + self.distance([p.x,p.y], [newnode.x,newnode.y]) < p.cost:
                # Show old lines here ->
                p.parent = newnode
    ...
    def RRT_planner(self, start_node, iterations = 500, info = None):
        ...
        start.info = self.underlying_distribution.distribution_function(node_C.reshape(-1, 2)) + np.random.normal(0, 1e-10)
