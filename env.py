...
from raor_parameters import NUM_AGENT
...
class Env():
    def __init__(self, sample_size=500, k_size=10, start=None, destination=None, obstacle=[], budget_range=None, save_image=False, seed=None):
        ...
        # self.start = np.random.rand(1, 2)
        # self.destination = np.random.rand(1, 2)
        self.global_node_coords = dict()
        for agent_ID in range(NUM_AGENT):
            self.global_node_coords[f"{agent_ID}"] = None
        
        self.global_graph = dict()
        for agent_ID in range(NUM_AGENT):
            self.global_graph[f"{agent_ID}"] = None
        ...
        for agent_ID in range(NUM_AGENT):
            ...
            self.global_node_coords[f"{agent_ID}"] = self.node_coords
            self.global_graph[f"{agent_ID}"] = self.graph
