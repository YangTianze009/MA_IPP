...
def calculate_intent_difference_abs(cov, cov_before, mean, mean_before):
    ...
    d = np.dstack([X, Y])
    Z1 = Gaussian.pdf(d).reshape(M, M) / Gaussian.pdf(mean)
    Z2 = Gaussian_before.pdf(d).reshape(M, M) / Gaussian_before.pdf(mean_before)
    for i in range(M):
        for j in range(M):
            intent_difference += abs(Z1[i][j] - Z2[i][j])
    return intent_difference
...
class Worker:
    ...
    def run_episode(self, currEpisode):
        global episode, agent_step, all_samples, cov_trace, gaussian_mean, gaussian_cov, first_step_seq, node_info, node_std, intent_difference
        ...
        # print(f"edge input is {edge_inputs}")
        try:
        ...
        for step_i in range(steps):
            ...
            # print(f"done is {done}", f"agent_ID is {self.agent_ID}", "\n")
            # print(f"end nodes is {sampling_end_nodes}")
            # print(f"gaussian mean is {gaussian_mean}")
            # print(f"gaussian cov is {gaussian_cov}")
            # print(f"intent input is {intent_input}")
            # print(f"node_coordinates are {self.node_coords}")
