...
def main():
    ...
    try:
        while True:
            ...
            print(f"cov_trace is {cov_trace_final_10}")
            ...
            while len(experience_buffer[0]) >= BATCH_SIZE:
                ...
                with torch.no_grad():
                    logp_list, value, _, _ = global_network(node_inputs_batch, edge_inputs_batch, budget_inputs_batch,
                                                            ...
                                                            pos_encoding_batch, agent_input_batch, mask_batch)
            ...
            if save_results:
                trainingData = []
                if curr_episode > 99:
                    if not os.path.exists("ma_ipp_results/10 agents/no_sampling_with_agent_input_0.25"):
                        os.makedirs("ma_ipp_results/10 agents/no_sampling_with_agent_input_0.25")
                    np.savez(f"ma_ipp_results/10 agents/no_sampling_with_agent_input_0.25/diff_graph_budget_2",
                             cov_trace_final_30)
