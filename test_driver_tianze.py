...
def main():
    ...
    try:
        while True:
            ...
            intent_difference_10.append(np.mean(perf_metrics["intent_difference_abs"], axis=0))
            print(f"intent difference 10 is {intent_difference_10}")
            ...
            save_cov_results = True
            save_intent_difference = False

            if not save_training_results:
            ...
            if len(cov_trace_final_10) == 3:
                ...
                cov_trace_final_10 = []
                intent_difference_30.append(np.mean(intent_difference_10))
                intent_difference_10 = []
            ...
            if save_cov_results:
                trainingData = []
                if curr_episode > 299:
                    if not os.path.exists("ma_ipp_results/3 agents/intent_end_nodes/intent_end_nodes_(8, 3)"):
                        os.makedirs(f"ma_ipp_results/3 agents/intent_end_nodes/intent_end_nodes_(8, 3)")
                    np.savez(f"ma_ipp_results/3 agents/intent_end_nodes/intent_end_nodes_(8, 3)/budget_2_0.1",
                             cov_trace_final_30)
