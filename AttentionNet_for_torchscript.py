...
class AttentionNet(nn.Module):
    ...
    def graph_embedding(self, node_inputs, edge_inputs, mask):
        ...
        for layer in self.nodes_update_layers:
            ...
            for i in range(sample_size):
                ...
                if i==0:
                    ...
                    connected_nodes_feature = torch.gather(embedding_feature, 1,
                                                           edge_inputs[:, i, :].unsqueeze(-1).repeat(1, 1,embedding_dim).long())
                # (batch, k_size, embedding_size)
                # print(connected_nodes_feature)
                    if mask.sum()>0:
                        node_mask = mask[:,i,:].unsqueeze(1)
                    else:
                        node_mask = mask[:,0,:].unsqueeze(1)
    ...
    def select_next_node(self, embedding_feature, edge_inputs, current_index, mask):
        # print(edge_inputs)
        # print(current_index.size())
        k_size = edge_inputs.size()[2]
        current_edge = torch.gather(edge_inputs, 1, current_index.repeat(1, 1, k_size).long())
        # print(current_edge)
        current_edge = current_edge.permute(0, 2, 1)
        embedding_dim = embedding_feature.size()[2]
        connected_nodes_feature = torch.gather(embedding_feature, 1, current_edge.repeat(1, 1, embedding_dim).long())
        # print(embedding_feature)
        # print(connected_nodes_feature)
        current_node_feature = torch.gather(embedding_feature, 1, current_index.repeat(1, 1, embedding_dim).long())
        ...
        if mask.sum()>0:
            current_mask = torch.gather(mask, 1, current_index.repeat(1,1,k_size).long())
            # print(current_mask)
        else:
            current_mask = mask[:, 0, :].unsqueeze(1)
        ...
        #if self.greedy:
        #    next_node_index = torch.argmax(logp_list, dim=1).long()
        #else:
        #    next_node_index = torch.multinomial(logp_list.exp(), 1).long().squeeze(1)

        #return next_node_index, logp_list, value
        return logp_list, value

    def forward(self, node_inputs, edge_inputs, current_index, mask):
        embedding_feature = self.graph_embedding(node_inputs, edge_inputs, mask)
        #next_node_index, logp_list, value = self.select_next_node(embedding_feature, edge_inputs, current_index, mask)
        #return next_node_index, logp_list, value
        
        logp_list, value = self.select_next_node(embedding_feature, edge_inputs, current_index, mask)
        return logp_list, value
...
def padding_inputs(inputs):
    ...
    # print(mask) # (1,sample_size+2,k_size)
    # print(seq.size())
    return seq, mask
...
if __name__ == '__main__':
    ...
    node_inputs = torch.torch.rand((1, 10, 2))
    # print(node_inputs)
    edge_inputs = torch.randint(0, 10, (1, 10, 5))
    edge_inputs_list = []
    ...
    current_index = torch.ones(size=(1, 1, 1), dtype=torch.int64)
    mask = torch.zeros((1,10,5), dtype=torch.int64)
    mask[0,2,1]=1
    print(1 in mask)
    logp_list= model(node_inputs, edge_inputs, current_index, mask)
    print(logp_list)
    scripted_module = torch.jit.script(model)
    #torch.jit.save(scripted_module, 'new_model.pt')
