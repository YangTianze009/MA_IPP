...
class RAOr:
    def __init__(self, sample_size=50, k_size=20, seed=None, show_img=False):
        ...
        self.env = Env(sample_size=sample_size, k_size=k_size, budget_range=BUDGET_RANGE, start=(0,0), destination=(1,1), seed=seed)
        ...
        for agent_ID in range(NUM_AGENT):
            self.global_agent_pos[f"{agent_ID}"] = [] 
        
        self.done = dict()
        for agent_ID in range(NUM_AGENT):
            self.done[f"{agent_ID}"] = False
    ...
    def make_plot(self, agent_ID, global_agent_pos):
        ...
        for agent_ID in range(NUM_AGENT):
            ...
            print(global_agent_pos[f"{agent_ID}"])
            pointsToDisplay = [path for path in global_agent_pos[f"{agent_ID}"]]
        ...
        plt.suptitle('Cov trace: {:.4g}'.format(self.env.cov_trace))
        # plt.tight_layout()
    ...
    def run_MARAOr_adaptive(self.):
        ...
        self.coords = self.env.global_node_coords[f"{agent_ID}"] * self.magnify
        # print(type(self.env.global_graph[f"{agent_ID}"]))
        self.graph = list(self.env.global_graph[f"{agent_ID}"].values())
        ...
        while True:
            ...
            cov_list.append(cov)
            print(f"current cov_trace is {cov}")
        ...
        #print(final_route)
        for route_i  in final_route:
            #print(route_i)
            self.global_agent_pos[f"{agent_ID}"].append(self.env.global_node_coords[f"{agent_ID}"][route_i])
        #print(self.global_agent_pos)
        
        self.make_plot(agent_ID=agent_ID, global_agent_pos= self.global_agent_pos)
        
        print('Global agent pos', self.global_agent_pos)
...
if __name__ == '__main__':
    ...
    for j in range(NUM_REPEAT):
        for i in range(NUM_TEST):
            ...
            covtr, _ = raor.run_MARAOr_adaptive()
            print(f"final cov_trace of agent is {covtr}")
