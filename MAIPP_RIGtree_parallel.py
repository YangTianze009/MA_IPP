...
class RIG_planner:
    ...
    def agent_replan(self, agent_ID):
        ...
        covariance_trace, if_done = None, None
        best_node = None
        length = 0
        ...
        while cost < REPLAN_LENGTH:
            ...
            # for i in connected_edges.items():
            #     plt.scatter()

            # old_cov_trace = pred_copy.cov_trace
            old_cov_trace = float("infinity")
            if path:
                dis_residual_prediction = best_dis_residual
            else:
                dis_residual_prediction = self.dist_residual[f"{agent_ID}"]
            # m = 0
            # print(f"old_cov_trace is {old_cov_trace}")
            for node, edge in connected_edges.items():
                if edge.length != 0.0 and node not in path:
                    # bug here
                    pred_copy = deepcopy(Predictor)
                    # print(f'true start position is {self.start[f"{agent_ID}"]}')
                    # print(f'false start position is {self.node_coords[int(start_node_index)]}')
                    cov_new, dist, distance_residual, new_measurements = pred_copy.prediction(node,
                                                                                              int(start_node_index),
                                                                                              dis_residual_prediction,
                                                                                              predict_measurements,
                                                                                              high_info_area)
                    # print(f"new_cov is {cov_new}")
                    # if cov_new < old_cov_trace:
                    if cov_new < old_cov_trace:
                        # if (old_cov_trace - cov_new) / dist > m:
                        # m = (old_cov_trace - cov_new) / dist
                        old_cov_trace = cov_new
                        best_dis_residual = distance_residual
            ...
            predict_measurements = best_measurements
            cost += length
        # start_index = 0
        ...
        for each_step in path:
            ...
            # print(f"cov_trace is {covariance_trace}")
            ...
            # print(f"global agent pos is {self.global_agent_pos}")
        ...
        self.start[f"{agent_ID}"] = self.global_agent_pos[f"{agent_ID}"][-1]
        # print(self.start)
        ...
        return covariance_trace, if_done

    def agent_planner(self):
        # Start node & init RRT
        self.start = dict()
        for i in range(NUM_AGENTS):
            self.start[f"{i}"] = np.array([START_X, START_Y])
        ...
        for agent_i in range(NUM_AGENTS):
            ...
            self.global_agent_pos[f"{agent_i}"].append(self.start[f"{agent_i}"])
        ...
        while self.all_done == False:
            ...
            # print(f"done is {self.agent_done}")

            covariance_trace, if_done = self.agent_replan(agent_ID=agent_ID)
            # self.agent_done[f"{agent_ID}"] = if_done
    ...
    def execute_path(self, each_step_pos, current_pos, agent_ID, sample_number, index_input=True):
        ...
        # print(f'distance is {BUDGET_SIZE - self.agent_budget[f"{agent_ID}"]}')
        # print(f'sample numbers are {len(self.measurement_points[f"{agent_ID}"])}')
